...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    char buf[256];
    if (IsTower(node))
        snprintf(buf, sizeof(buf), GetMaNGOSString(LANG_BG_AV_TOWER_TAKEN) , GetNodeName(node), (owner == ALLIANCE) ? GetMaNGOSString(LANG_BG_AV_ALLY) : GetMaNGOSString(LANG_BG_AV_HORDE));
    else
        snprintf(buf, sizeof(buf), GetMaNGOSString(LANG_BG_AV_GRAVE_TAKEN) , GetNodeName(node), (owner == ALLIANCE) ? GetMaNGOSString(LANG_BG_AV_ALLY) : GetMaNGOSString(LANG_BG_AV_HORDE));
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, uint32 team, bool initial)
{
    ...
    if (team == ALLIANCE || team == HORDE)
    {
        ...
        char buf[256];
        snprintf(buf, sizeof(buf), GetMaNGOSString(LANG_BG_AV_MINE_TAKEN), GetMaNGOSString((mine == BG_AV_NORTH_MINE) ? LANG_BG_AV_MINE_NORTH : LANG_BG_AV_MINE_SOUTH), (team == ALLIANCE) ?  GetMaNGOSString(LANG_BG_AV_ALLY) : GetMaNGOSString(LANG_BG_AV_HORDE));
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
        if (creature)
            YellToAll(creature, buf, LANG_UNIVERSAL);
    }
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    //send a nice message to all :)
    char buf[256];
    snprintf(buf, sizeof(buf), GetMaNGOSString((IsTower(node)) ? LANG_BG_AV_TOWER_DEFENDED : LANG_BG_AV_GRAVE_DEFENDED), GetNodeName(node), (team == ALLIANCE) ?  GetMaNGOSString(LANG_BG_AV_ALLY) : GetMaNGOSString(LANG_BG_AV_HORDE));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
    if (creature)
        YellToAll(creature, buf, LANG_UNIVERSAL);
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    //send a nice message to all :)
    char buf[256];
    snprintf(buf, sizeof(buf), (IsTower(node)) ? GetMaNGOSString(LANG_BG_AV_TOWER_ASSAULTED) : GetMaNGOSString(LANG_BG_AV_GRAVE_ASSAULTED), GetNodeName(node),  (team == ALLIANCE) ?  GetMaNGOSString(LANG_BG_AV_ALLY) : GetMaNGOSString(LANG_BG_AV_HORDE));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
    if (creature)
        YellToAll(creature, buf, LANG_UNIVERSAL);
...
const char* BattlegroundAV::GetNodeName(BG_AV_Nodes node)
{
    ...
    case BG_AV_NODES_FIRSTAID_STATION:
        return GetMaNGOSString(LANG_BG_AV_NODE_GRAVE_STORM_AID);
    case BG_AV_NODES_DUNBALDAR_SOUTH:
        return GetMaNGOSString(LANG_BG_AV_NODE_TOWER_DUN_S);
    case BG_AV_NODES_DUNBALDAR_NORTH:
        return GetMaNGOSString(LANG_BG_AV_NODE_TOWER_DUN_N);
    case BG_AV_NODES_STORMPIKE_GRAVE:
        return GetMaNGOSString(LANG_BG_AV_NODE_GRAVE_STORMPIKE);
    case BG_AV_NODES_ICEWING_BUNKER:
        return GetMaNGOSString(LANG_BG_AV_NODE_TOWER_ICEWING);
    case BG_AV_NODES_STONEHEART_GRAVE:
        return GetMaNGOSString(LANG_BG_AV_NODE_GRAVE_STONE);
    case BG_AV_NODES_STONEHEART_BUNKER:
        return GetMaNGOSString(LANG_BG_AV_NODE_TOWER_STONE);
    case BG_AV_NODES_SNOWFALL_GRAVE:
        return GetMaNGOSString(LANG_BG_AV_NODE_GRAVE_SNOW);
    case BG_AV_NODES_ICEBLOOD_TOWER:
        return GetMaNGOSString(LANG_BG_AV_NODE_TOWER_ICE);
    case BG_AV_NODES_ICEBLOOD_GRAVE:
        return GetMaNGOSString(LANG_BG_AV_NODE_GRAVE_ICE);
    case BG_AV_NODES_TOWER_POINT:
        return GetMaNGOSString(LANG_BG_AV_NODE_TOWER_POINT);
    case BG_AV_NODES_FROSTWOLF_GRAVE:
        return GetMaNGOSString(LANG_BG_AV_NODE_GRAVE_FROST);
    case BG_AV_NODES_FROSTWOLF_ETOWER:
        return GetMaNGOSString(LANG_BG_AV_NODE_TOWER_FROST_E);
    case BG_AV_NODES_FROSTWOLF_WTOWER:
        return GetMaNGOSString(LANG_BG_AV_NODE_TOWER_FROST_W);
    case BG_AV_NODES_FROSTWOLF_HUT:
        return GetMaNGOSString(LANG_BG_AV_NODE_GRAVE_FROST_HUT);
    default:
