...
public:
    ...
    struct npc_muglashAI : public npc_escortAI
    {
        ...
        bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest)
        {
            ...
            {
                DoScriptText(SAY_MUG_START1, me, player);
                me->SetFaction(FACTION_ESCORT_H_PASSIVE);
...
public:
    ...
    {
        if (Creature* pCreature = GetClosestCreatureWithEntry(go, NPC_MUGLASH, INTERACTION_DISTANCE * 2))
        {
...
{
public:
    npc_freero() : CreatureScript("npc_feero") {}
