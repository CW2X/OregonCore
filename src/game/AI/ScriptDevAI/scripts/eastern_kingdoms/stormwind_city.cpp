...
public:
	...
	struct npc_lord_gregor_lescovarAI : public npc_escortAI
	{
		...
		void DoGuardList()
		{
			...
			MaNGOS::AllCreaturesOfEntryInRange check(me, NPC_STORMWIND_ROYAL, 10);
			MaNGOS::CreatureListSearcher<MaNGOS::AllCreaturesOfEntryInRange> searcher(lCreatureList, check);
			...
			if (!lCreatureList.empty())
			{
				...
				{
					CAST_AI(npc_stormwind_royal_guard::npc_stormwind_royal_guardAI, (*itr)->AI())->MovePoint = 1;
					CAST_AI(npc_stormwind_royal_guard::npc_stormwind_royal_guardAI, (*itr)->AI())->PointID = PointID;
					PointID++;
				}
			}
		}
	...
	struct npc_tyrion_spybotAI : public npc_escortAI
	{
		...
		void DoGuardList()
		{
			...
			if (!lCreatureList.empty())
			{
				for (std::list<Creature*>::iterator itr = lCreatureList.begin(); itr != lCreatureList.end(); ++itr)
				{
					CAST_AI(npc_stormwind_royal_guard::npc_stormwind_royal_guardAI, (*itr)->AI())->SpybotGUID = me->GetGUID();
					CAST_AI(npc_stormwind_royal_guard::npc_stormwind_royal_guardAI, (*itr)->AI())->uiSpybotPhase = 1;
				}
			}
		}
    ...
    bool OnQuestAccept(Player* pPlayer, Creature* pCreature, Quest const* pQuest) override
    {
        if (pQuest->GetQuestId() == QUEST_THE_ATTACK)
        {
            if (Creature* pSpybot = pCreature->FindNearestCreature(NPC_TYRION_SPYBOT, 5.0f, true))
            {
                ...
                CAST_AI(npc_tyrion_spybot::npc_tyrion_spybotAI, pSpybot->AI())->SetEscortPaused(true);
                CAST_AI(npc_tyrion_spybot::npc_tyrion_spybotAI, pSpybot->AI())->uiPhase = 1;
                CAST_AI(npc_tyrion_spybot::npc_tyrion_spybotAI, pSpybot->AI())->uiTimer = 200;
...
public: 
    ...
    struct npc_marzon_silent_bladeAI : public ScriptedAI
    {
        ...
        void MovementInform(uint32 uiType, uint32 /*uiId*/)
        {
            ...
            {
                CAST_AI(npc_lord_gregor_lescovar::npc_lord_gregor_lescovarAI, pLescovar->AI())->uiTimer = 2000;
                CAST_AI(npc_lord_gregor_lescovar::npc_lord_gregor_lescovarAI, pLescovar->AI())->uiPhase = 5;
            }
        }
