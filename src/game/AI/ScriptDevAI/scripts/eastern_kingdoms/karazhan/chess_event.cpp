    ...
    struct chess_npcAI : public Scripted_NoMovementAI
    {
        ...
        void JustDied(Unit* Killer)
        {
            ...
            if (me->GetEntry() == NPC_KING_H || me->GetEntry() == NPC_KING_A)
            {
                ...
                npc_echo_of_medivh::npc_echo_of_medivhAI* medivhAI = (npc_echo_of_medivh::npc_echo_of_medivhAI*)GetMedivhAI(pInstance, me);
                if (medivhAI)
...
public:
    ...
    struct Move_markerAI : public ScriptedAI
    {
        ...
        {
            chess_npc::chess_npcAI* chessAI = (chess_npc::chess_npcAI*)pSummoner->ToCreature()->AI();
...
public:
    ...
    struct chess_victory_controlerAI : ScriptedAI
    {
        ...
        chess_victory_controlerAI(Creature* c) : ScriptedAI(c)
        {
            pInstance = (ScriptedInstance*)c->GetInstanceData();
            Reset();
        }
