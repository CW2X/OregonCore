...
static const unsigned int sMaNGOSLogoRows = 9;
static const unsigned int sMaNGOSLogoCols = 56;
...
void Console::Initialize()
{
    ...
    if (!m_logoWindow)
    {
        ...
        m_logoWindow = MakeWindow(sMaNGOSLogoRows, sMaNGOSLogoCols, 1,                               COLS / 2 - sMaNGOSLogoCols / 2);
        m_loadWindow = MakeWindow(3,               sMaNGOSLogoCols, 2 + sMaNGOSLogoRows,             COLS / 2 - sMaNGOSLogoCols / 2);
...
void Console::MainLoop()
{
    ...
    //                                height            width                    y                         x
    Window* menuWindow = MakeWindow(MenuItems + 2,     19,              1 + sMaNGOSLogoRows + 1,         COLS / 2 - 10 - 2);
    Window* infoWindow = MakeWindow(4,               sMaNGOSLogoCols, 5 + sMaNGOSLogoRows + MenuItems, COLS / 2 - sMaNGOSLogoCols / 2);
    ...
    while (!World::IsStopped())
    {
        ...
        case KEY_RESIZE:
            ...
            //           win             height         width                 y                            x
            ResizeWindow(m_logoWindow, sMaNGOSLogoRows, sMaNGOSLogoCols, 1,                       COLS / 2 - sMaNGOSLogoCols / 2);
            ResizeWindow(menuWindow, MenuItems + 2,     19,              1 + sMaNGOSLogoRows + 1, std::max<int>(0, COLS / 2 - 10 - 2));
            ResizeWindow(infoWindow, 4,               sMaNGOSLogoCols, 5 + sMaNGOSLogoRows + MenuItems, COLS / 2 - sMaNGOSLogoCols / 2);
...
void Console::RunCommandLoop()
{
    ...
    int lineWidth = COLS - (sizeof("MaNGOS>") - 1);
    Window* cmdLine = MakeWindow(1, lineWidth, LINES - 1, (sizeof("MaNGOS>") - 1));

    erase();
    curs_set(1);

    attron(A_BOLD | TermColor(COLOR_GREEN));
    mvprintw(LINES - 1, 0, "MaNGOS> ");
    ...
    while (!World::IsStopped())
    {
        ...
        {
            lineWidth = COLS - (sizeof("MaNGOS>") - 1);

            wresize(m_cmdOutput, 0xFFFF / COLS, COLS);

            ResizeWindow(cmdLine, 1, lineWidth, LINES - 1, (sizeof("MaNGOS>") - 1));
...
void Console::SetLoading(bool on, const char* caption)
{
    ...
    if (on)
    {
        ...
        mvwprintw(m_loadWindow, 0, sMaNGOSLogoCols - (sizeof(stop) - 1), "%s", stop);
...
void Console::FatalError(const char* msg)
{
    ...
    Window* errWin = MakeWindow(LINES - (2 + sMaNGOSLogoRows), sMaNGOSLogoCols, 2 + sMaNGOSLogoRows, COLS / 2 - sMaNGOSLogoCols / 2);
...
void Console::DrawLogo()
{
    ...
    if (m_IamEnabled)
    {
        ...
        mvwprintw(m_logoWindow, 0, 0, "%s", sMaNGOSLogo);
        wattroff(m_logoWindow, TermColor(COLOR_GREEN));
        wattron(m_logoWindow, TermColor(COLOR_CYAN));

        mvwprintw(m_logoWindow, 1, sMaNGOSLogoCols / 2 - (sizeof(FavString) - 1) / 2, "%s", FavString);
        mvwprintw(m_logoWindow, sMaNGOSLogoRows - 1, 0, "%s", "https://www.mangos-core.net");
    ...
    {
        sLog.outString();
        for (uint32 i = 0; i < sMaNGOSLogoRows; i++)
            sLog.outString("%.*s", sMaNGOSLogoCols, &sMaNGOSLogo[i * sMaNGOSLogoCols]);
        sLog.outString("https://www.mangos-core.net\n");
    }
}
