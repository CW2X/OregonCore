...
int Master::Run()
{
    ...
    // set realmbuilds depend on MaNGOSCore expected builds, and set server online
    ...
    #ifdef _WIN32
    {
        ...
        if (Aff > 0)
        {
            ...
            if (GetProcessAffinityMask(hProcess, &appAff, &sysAff))
            {
                ...
                if (!curAff)
                    sLog.outError("Processors marked in UseProcessors bitmask (hex) %x not accessible for MaNGOSCore. Accessible processors bitmask (hex): %x", Aff, appAff);
                else
                {
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                sLog.outString("MaNGOSCore process priority class set to HIGH");
            else
                sLog.outError("ERROR: Can't set MaNGOSCore process priority class.");
            sLog.outString();
        }
    }
    #endif
